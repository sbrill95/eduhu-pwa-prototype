...
describe('PromptService', () => {
  ...
  describe('generateSuggestions', () => {
    ...
    test('should personalize prompts with user profile data', async () => {
      ...
      const hasExpectedSubject = result.some(s => s.prompt.includes('Mathematik'));
    ...
    test('should use fallback profile when user profile not found', async () => {
      ...
      (InstantDBService.TeacherProfileService.getTeacherProfile as jest.Mock).mockResolvedValue(null);
    ...
    test('should exclude templates with matching excludeIds', async () => {
      ...
      const excludeIds = firstResult.slice(0, 3).map(s => s.metadata?.templateId || '');
  ...
  describe('seededShuffle', () => {
    ...
    test('should produce different order with different seeds', async () => {
      ...
      const hasDifference = templateIds1.some((id, index) => id !== templateIds2[index]);
  ...
  describe('fallback topic generation', () => {
    test('should generate appropriate topics for different subjects', async () => {
      ...
      // Test Mathematik
      (InstantDBService.TeacherProfileService.getTeacherProfile as jest.Mock).mockResolvedValue({
        subjects: ['Mathematik'],
        grades: ['7'],
      ...
      const hasMathTopics = mathResult.some(s =>
      ...
      const hasGermanTopics = germanResult.some(s =>
    ...
    test('should use user topics if available', async () => {
      ...
      const hasUserTopics = result.some(s =>
