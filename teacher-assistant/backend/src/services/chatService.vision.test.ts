...
describe('ChatService Vision Integration', () => {
  describe('processMessagesForVision', () => {
    it('should detect and format messages with image metadata', () => {
      ...
      const processMessagesForVision = (ChatService as any).processMessagesForVision.bind(ChatService);
      ...
      expect(result.messages[0].content).toBe('Erstelle ein Bild von einem Löwen');
      ...
      expect(result.messages[1].content[0].text).toBe('Ich habe ein Bild für dich erstellt.');
      expect(result.messages[1].content[1].type).toBe('image_url');
      expect(result.messages[1].content[1].image_url.url).toBe('https://example.com/lion.png');
