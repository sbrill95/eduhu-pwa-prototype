...
describe('ProfileExtractionService', () => {
  ...
  describe('extractCharacteristics', () => {
    it('should extract 2-3 characteristics from valid chat', async () => {
      ...
      const messages: ChatMessage[] = [
        ...
        {
          role: 'assistant',
          content: 'Gerne! Was für ein Thema behandeln Sie gerade?',
        },
        {
          role: 'user',
          content:
            'Wir arbeiten an Bruchrechnung. Die Schüler haben Schwierigkeiten mit dem Kürzen.',
        },
      ...
      (
        InstantDBService.ProfileCharacteristics
          .incrementCharacteristic as jest.Mock
      ).mockResolvedValue(undefined);

      const result = await service.extractCharacteristics(
        mockUserId,
        messages,
        existingProfile
      );
      ...
      expect(result[2]).toEqual({
        characteristic: 'Bruchrechnung',
        category: 'topics',
      });

      // Verify InstantDB was called for each characteristic
      expect(
        InstantDBService.ProfileCharacteristics.incrementCharacteristic
      ).toHaveBeenCalledTimes(3);
    });

    it('should handle empty/short chats gracefully', async () => {
      const emptyMessages: ChatMessage[] = [];
      const result1 = await service.extractCharacteristics(
        mockUserId,
        emptyMessages,
        []
      );
      ...
      const shortMessages: ChatMessage[] = [{ role: 'user', content: 'Hallo' }];
      const result2 = await service.extractCharacteristics(
        mockUserId,
        shortMessages,
        []
      );
      ...
      const messages: ChatMessage[] = [
        {
          role: 'user',
          content:
            'Ich unterrichte Mathematik, Deutsch, Englisch und Geschichte',
        },
        {
          role: 'assistant',
          content: 'Das ist eine vielfältige Fächerkombination!',
        },
      ];
          ...
          choices: [
            {
              message: {
                content:
                  '["Mathematik", "Deutsch", "Englisch", "Geschichte", "Klasse 8"]',
              },
            },
          ],
        })
    ...
    it('should avoid duplicate characteristics', async () => {
      ...
      expect(
        InstantDBService.ProfileCharacteristics.incrementCharacteristic
      ).toHaveBeenCalledWith(mockUserId, 'Mathematik', 'subjects');
    });
  });
  ...
  describe('buildExtractionPrompt', () => {
    it('should build prompt with conversation and existing profile', () => {
      ...
      const prompt = (service as any).buildExtractionPrompt(
        messages,
        existingProfile
      );
    ...
    it('should handle empty existing profile', () => {
      const messages: ChatMessage[] = [{ role: 'user', content: 'Hallo' }];
