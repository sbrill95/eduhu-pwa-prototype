...
router.get('/data/subjects', [
  ...
  try {
    ...
    if (grade_level && typeof grade_level === 'string') {
      subjects = subjects.filter(subject =>
        subject.grade_levels.includes(grade_level)
      );
    }
...
router.get('/data/preferences', [
  ...
  try {
    ...
    const categories = [...new Set(allPreferences.map(p => p.category))].sort();
 ...
 */
router.post('/data/seed', async (req: Request, res: Response): Promise<void> => {
  try {
...
router.get('/data/search', [
  ...
  try {
    ...
    const searchTypes = typeof types === 'string' ? types.split(',') : ['states', 'subjects', 'preferences'];
    ...
    if (searchTypes.includes('states')) {
      ...
      results.states = dataSeederService.searchData(states, searchTerm as string).slice(0, 10);
    }

    // Search subjects
    if (searchTypes.includes('subjects')) {
      const subjects = await dataSeederService.getTeachingSubjects();
      results.subjects = dataSeederService.searchData(subjects, searchTerm as string).slice(0, 10);
    }

    // Search preferences
    if (searchTypes.includes('preferences')) {
      const preferences = await dataSeederService.getTeachingPreferences();
      results.preferences = dataSeederService.searchData(preferences, searchTerm as string).slice(0, 10);
    }

    const totalResults = (results.states?.length || 0) +
