...
async function processFileUpload(req: Request, res: Response): Promise<void> {
  try {
    if (!req.file) {
      logError('File upload failed: No file provided', new Error('No file uploaded'));
      res.status(400).json({
        success: false,
        error: 'No file uploaded.',
        user_message: 'Keine Datei hochgeladen. Bitte wählen Sie eine Datei aus.'
      });
      return;
    }
    ...
    if (validationError) {
      logError(`File validation failed: ${validationError}`, new Error(validationError));
      res.status(400).json({
        success: false,
    ...
    logInfo(`Processing file upload: ${req.file.originalname} (${req.file.size} bytes)`);
    ...
    logError(`File upload error: ${errorMessage}`, error instanceof Error ? error : new Error(String(error)));
    ...
    if (errorMessage.includes('exceeds the maximum allowed size')) {
      res.status(400).json({
        ...
        user_message: 'Die Datei ist zu groß für den Upload. Maximale Größe: 10MB.'
      });
      return;
    }
    ...
    if (errorMessage.includes('unsupported file type')) {
      res.status(400).json({
        ...
        user_message: 'Dateityp nicht unterstützt. Erlaubt: JPG, PNG, GIF, PDF, DOCX, DOC, TXT.'
      });
      return;
    }
    ...
    res.status(500).json({
      ...
      user_message: 'Fehler beim Hochladen der Datei. Bitte versuchen Sie es erneut.'
    });
  }
}
