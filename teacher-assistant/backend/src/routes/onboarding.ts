...
router.post('/onboarding', [
  ...
  body('subjects').isArray().custom((subjects: string[]) => {
  ...
  body('teachingPreferences').isArray().custom((preferences: string[]) => {
  ...
  try {
    ...
    const existingUser = await instantdbService.db.query({
      users: {
        $: {
          where: { id: userId }
        }
    ...
    if (existingUser.data?.users?.length) {
      ...
      const updateData: Partial<User> = {
        ...
        grade_levels: JSON.stringify([gradeLevel]),
        teaching_preferences: JSON.stringify(teachingPreferences),
      ...
      await instantdbService.db.transact(
        instantdbService.db.tx.users[userId].update(updateData)
      );
    } else {
      // Create new user
      ...
      await instantdbService.db.transact(
        instantdbService.db.tx.users.insert({ ...newUser, id: userId })
      );
    }

    // Also create or update teacher profile
    const existingProfile = await instantdbService.db.query({
      teacher_profiles: {
        $: {
    ...
    if (existingProfile.data?.teacher_profiles?.length) {
      ...
      await instantdbService.db.transact(
        instantdbService.db.tx.teacher_profiles[existingProfile.data.teacher_profiles[0].id].update({
          display_name: name,
          subjects: JSON.stringify(subjects),
      ...
      // Create new profile
      await instantdbService.db.transact(
        instantdbService.db.tx.teacher_profiles.insert({
          user_id: userId,
          display_name: name,
          subjects: JSON.stringify(subjects),
          grades: JSON.stringify([gradeLevel]),
 ...
 */
router.put('/onboarding/:userId', [
  body('germanState').optional().isString().trim().isLength({ min: 1, max: 50 }),
  body('subjects').optional().isArray().custom((subjects: string[]) => {
    if (subjects && (!Array.isArray(subjects) || subjects.length === 0)) {
      throw new Error('Subjects must be a non-empty array');
    }
    if (subjects && subjects.some(subject => typeof subject !== 'string' || subject.trim() === '')) {
      throw new Error('All subjects must be non-empty strings');
    }
    return true;
  }),
  body('gradeLevel').optional().isString().trim().isLength({ min: 1, max: 20 }),
  body('teachingPreferences').optional().isArray().custom((preferences: string[]) => {
    if (preferences && !Array.isArray(preferences)) {
      throw new Error('Teaching preferences must be an array');
    }
    if (preferences && preferences.some(pref => typeof pref !== 'string' || pref.trim() === '')) {
      throw new Error('All teaching preferences must be non-empty strings');
    }
    return true;
  }),
  ...
  try {
    ...
    const updateData: Partial<User> = {
      ...
      ...(gradeLevel && { grade_levels: JSON.stringify([gradeLevel]) }),
      ...(teachingPreferences && { teaching_preferences: JSON.stringify(teachingPreferences) }),
    ...
    if (existingProfile.data?.teacher_profiles?.length) {
      const profileUpdateData: any = {
        ...
        ...(teachingPreferences && { teaching_methods: JSON.stringify(teachingPreferences) })
      };

      await instantdbService.db.transact(
        instantdbService.db.tx.teacher_profiles[existingProfile.data.teacher_profiles[0].id].update(profileUpdateData)
      );
    }
...
router.get('/onboarding/:userId', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const onboardingData = {
      ...
      gradeLevel: userData.grade_levels ? JSON.parse(userData.grade_levels)[0] : null,
      teachingPreferences: userData.teaching_preferences ? JSON.parse(userData.teaching_preferences) : [],
