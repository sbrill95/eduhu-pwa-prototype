 ...
 */
router.get('/profile/context/:userId', [
  param('userId').isString().notEmpty(),
  query('contextType').optional().isIn(['subject', 'grade', 'method', 'topic', 'challenge', 'custom']),
  ...
  try {
    ...
    const contexts = (contextData.data?.manual_context || []).map(context => ({
      id: context.id,
    ...
    const response: ApiResponse<{
      ...
      data: {
        ...
        grouped: contexts.reduce((acc, context) => {
...
router.post('/profile/context', [
  ...
  body('contextType').isIn(['subject', 'grade', 'method', 'topic', 'challenge', 'custom']),
  ...
  try {
    ...
    const newContext: Omit<ManualContext, 'id' | 'user'> = {
 ...
 */
router.put('/profile/context/:contextId', [
  param('contextId').isString().notEmpty(),
  body('content').optional().isString().trim().isLength({ min: 1, max: 500 }),
  body('contextType').optional().isIn(['subject', 'grade', 'method', 'topic', 'challenge', 'custom']),
  body('priority').optional().isInt({ min: 1, max: 10 }),
 ...
 */
router.delete('/profile/context/:contextId', [
  param('contextId').isString().notEmpty(),
 ...
 */
router.post('/profile/context/bulk', [
  body('userId').isString().notEmpty(),
  body('operation').isIn(['create', 'update', 'delete', 'activate', 'deactivate']),
    ...
    switch (operation) {
      ...
      case 'deactivate':
        if (!contextIds || !Array.isArray(contextIds)) {
          throw new Error('Context IDs array is required for activation/deactivation');
        }
      ...
      case 'delete':
        if (!contextIds || !Array.isArray(contextIds)) {
          throw new Error('Context IDs array is required for delete operation');
        }
