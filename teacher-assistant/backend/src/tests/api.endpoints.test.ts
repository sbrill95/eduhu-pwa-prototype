...
describe.skip('LangGraph API Endpoints', () => {
  ...
  describe('GET /api/langgraph-agents/image/usage/:userId', () => {
    test('should return usage information for valid user', async () => {
      ...
      expect(response.body.data.remaining).toBeLessThanOrEqual(
        response.body.data.monthly_limit
      );
    });
    ...
    test('should return WebSocket connection information', async () => {
      ...
      expect(response.body).toEqual({
        ...
        data: {
          ...
          supported_levels: expect.arrayContaining([
            'user_friendly',
            'detailed',
            'debug',
          ]),
  ...
  describe('Response Format Consistency', () => {
    test('all endpoints should return consistent error format', async () => {
      const endpoints = [
        ...
        {
          method: 'get',
          path: '/api/langgraph-agents/execution/invalid/status',
        },
        {
          method: 'post',
          path: '/api/langgraph-agents/execution/invalid/cancel',
          body: {},
        },
      ];

      for (const endpoint of endpoints) {
        const request_call = request(app)[endpoint.method as 'get' | 'post'](
          endpoint.path
        );
    ...
    test('all successful endpoints should return consistent success format', async () => {
      const endpoints = [
        ...
        {
          method: 'get',
          path: `/api/langgraph-agents/image/usage/${testUserId}`,
        },
  ...
  describe('HTTP Status Codes', () => {
    ...
    test('should return 200 for successful requests', async () => {
      await request(app).get('/api/langgraph-agents/available').expect(200);
    });
  });
