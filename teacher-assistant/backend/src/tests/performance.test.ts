...
describe('Performance and Load Testing', () => {
  ...
  afterAll(() => {
    ...
    Object.keys(performanceMetrics).forEach((operation) => {
      ...
      console.log(`  Min: ${(stats.min ?? 0).toFixed(2)}ms`);
      console.log(`  Max: ${(stats.max ?? 0).toFixed(2)}ms`);
      console.log(`  95th percentile: ${(stats.p95 ?? 0).toFixed(2)}ms`);
    });
  });
  ...
  describe('API Response Time Performance', () => {
    test('agent discovery should respond quickly', async () => {
      ...
      console.log(
        `Agent discovery average response time: ${avgTime.toFixed(2)}ms`
      );
    ...
    test.skip('usage tracking should respond quickly', async () => {
      const iterations = 5;
      const times: number[] = [];
      ...
      const avgTime = times.reduce((a, b) => a + b, 0) / times.length;
      console.log(
        `Usage tracking average response time: ${avgTime.toFixed(2)}ms`
      );
    ...
    test.skip('artifact retrieval should respond quickly', async () => {
      const iterations = 5;
      const times: number[] = [];
      ...
      const avgTime = times.reduce((a, b) => a + b, 0) / times.length;
      console.log(
        `Artifact retrieval average response time: ${avgTime.toFixed(2)}ms`
      );
    ...
    test('execution status should respond quickly', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        const response = await request(app).get(
          `/api/langgraph-agents/execution/test-exec-${i}/status`
        );
      ...
      const avgTime = times.reduce((a, b) => a + b, 0) / times.length;
      console.log(
        `Execution status average response time: ${avgTime.toFixed(2)}ms`
      );
  ...
  describe('Concurrent Request Performance', () => {
    test('should handle concurrent agent discovery requests', async () => {
      ...
      const promises = Array.from({ length: concurrentRequests }, (_, i) =>
        request(app).get('/api/langgraph-agents/available').expect(200)
      );
      ...
      console.log(
        `${concurrentRequests} concurrent requests completed in ${totalDuration.toFixed(2)}ms`
      );
    ...
    test.skip('should handle concurrent usage tracking requests', async () => {
      ...
      const promises = Array.from({ length: concurrentRequests }, (_, i) =>
        request(app)
          .get(
            `/api/langgraph-agents/image/usage/${testUserId}-concurrent-${i}`
          )
          .expect(200)
      );
      ...
      console.log(
        `${concurrentRequests} concurrent usage requests completed in ${totalDuration.toFixed(2)}ms`
      );
    ...
    test.skip('should handle mixed concurrent requests', async () => {
      ...
      for (let i = 0; i < concurrentRequests; i++) {
        if (i % 3 === 0) {
          promises.push(request(app).get('/api/langgraph-agents/available'));
        } else if (i % 3 === 1) {
          promises.push(
            request(app).get(
              `/api/langgraph-agents/image/usage/${testUserId}-mixed-${i}`
            )
          );
        } else {
          promises.push(
            request(app).get(`/api/agents/artifacts/${testUserId}-mixed-${i}`)
          );
        }
      }
      ...
      console.log(
        `${concurrentRequests} mixed concurrent requests completed in ${totalDuration.toFixed(2)}ms`
      );
  ...
  describe('Memory and Resource Performance', () => {
    test('should handle large parameter payloads efficiently', async () => {
      ...
      console.log(
        `Large payload request completed in ${duration.toFixed(2)}ms`
      );
    ...
    test.skip('should handle many small requests efficiently', async () => {
      const numberOfRequests = 50;
      const startTime = performance.now();

      const promises = Array.from({ length: numberOfRequests }, (_, i) =>
        request(app).get(
          `/api/langgraph-agents/image/usage/${testUserId}-small-${i}`
        )
      );
      ...
      const successfulResponses = responses.filter((r) => r.status === 200);
      expect(successfulResponses.length).toBeGreaterThan(
        numberOfRequests * 0.9
      );

      console.log(
        `${numberOfRequests} small requests: ${totalDuration.toFixed(2)}ms total, ${avgDuration.toFixed(2)}ms average`
      );
  ...
  describe('Stress Testing', () => {
    test.skip('should maintain performance under sustained load', async () => {
      ...
      console.log(
        `Stress test: ${requestCount} requests over ${actualDuration.toFixed(0)}ms`
      );
    ...
    test('should recover from peak load scenarios', async () => {
      ...
      const burstPromises = Array.from({ length: burstSize }, (_, i) =>
        request(app).get('/api/langgraph-agents/available')
      );
      ...
      const burstSuccessRate =
        burstResponses.filter((r) => r.status === 200).length / burstSize;

      // Normal load after peak should succeed
      expect(normalResponses).toHaveLength(normalSize);

      console.log(
        `Peak load burst: ${burstDuration.toFixed(2)}ms, success rate: ${(burstSuccessRate * 100).toFixed(1)}%`
      );
  ...
  describe('Database Performance', () => {
    test.skip('should handle usage tracking queries efficiently', async () => {
      const users = Array.from(
        { length: 20 },
        (_, i) => `perf-db-user-${i}-${Date.now()}`
      );
      ...
      console.log(
        `Database usage queries: ${totalDuration.toFixed(2)}ms total, ${avgDuration.toFixed(2)}ms average`
      );
    ...
    test.skip('should handle artifact queries efficiently', async () => {
      const users = Array.from(
        { length: 10 },
        (_, i) => `perf-artifact-user-${i}-${Date.now()}`
      );
      ...
      console.log(
        `Database artifact queries: ${totalDuration.toFixed(2)}ms total, ${avgDuration.toFixed(2)}ms average`
      );
    ...
    test('should meet performance SLA requirements', () => {
      ...
      Object.keys(requirements).forEach((operation) => {
        ...
        if (stats.avg > 0) {
          ...
          console.log(
            `${operation}: avg ${stats.avg.toFixed(2)}ms (req: <${req.max_avg}ms), p95 ${(stats.p95 ?? 0).toFixed(2)}ms (req: <${req.max_p95}ms)`
          );

          expect(stats.avg).toBeLessThan(req.max_avg);
          expect(stats.p95 ?? 0).toBeLessThan(req.max_p95);
        }
      });
    });
