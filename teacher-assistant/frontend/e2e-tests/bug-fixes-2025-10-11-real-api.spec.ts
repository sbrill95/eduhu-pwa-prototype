...
/**
 * E2E Test Suite: Bug Fixes 2025-10-11 (REAL API MODE - Integration Tests)
 *
 * @real-api - This suite calls actual OpenAI DALL-E 3 API
 *
 * Comprehensive integration tests for 4 user stories:
 ...
 *
 * Test Strategy:
 * - REAL API MODE: Calls actual OpenAI DALL-E 3 (slow, 60-90s per image)
 ...
 *
 * WARNING: This suite takes 5-10 minutes to run and incurs API costs
 * Run selectively for smoke testing and integration validation
 *
 * Tasks: T043-T052
...
class BugFixTestHelper {
  ...
  async generateImage(description: string) {
    ...
    // Wait for agent confirmation (REAL API - may take 30-60s)
    console.log('⏳ Waiting for agent suggestion (REAL API - may take 30-60s for DALL-E 3)...');
    ...
    // Wait for result (REAL API - longer timeout for actual DALL-E 3)
    console.log('⏳ Waiting for image generation result (REAL API - DALL-E 3 is processing)...');
    const resultView = this.page.locator('[data-testid="agent-result-view"], .agent-result-view').first();
    await expect(resultView).toBeVisible({ timeout: 120000 }); // Wait up to 2 minutes for real image
...
test.describe('Bug Fixes 2025-10-11 - E2E Test Suite', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // NO MOCK SERVER - Using real APIs
    console.log('🚀 REAL API MODE - Calls will go to actual OpenAI DALL-E 3');
  ...
  test('US1 (BUG-030): "Weiter im Chat" navigates to Chat tab with image thumbnail', async ({ page }) => {
    test.setTimeout(300000); // 5 minutes (REAL API - DALL-E 3 is slow)
    ...
    // CORRECT SELECTOR: ChatView uses .flex.mb-3 for message wrappers, .rounded-2xl for bubbles
    const chatMessages = page.locator('.flex.mb-3');
  ...
  test('US1 (BUG-030): Debouncing prevents duplicate navigation on rapid clicks', async ({ page }) => {
    ...
    console.log('\n🎯 TEST: US1 (BUG-030) - Debouncing [REAL API]');
  ...
  test('US2 (BUG-025): Messages persist with metadata after page refresh', async ({ page }) => {
    ...
    console.log('\n🎯 TEST: US2 (BUG-025) - Message Persistence [REAL API]');
  ...
  test('US3 (BUG-020): Library displays materials in grid (no placeholder)', async ({ page }) => {
    test.setTimeout(600000); // 10 minutes (REAL API - 3 images with DALL-E 3)

    console.log('\n🎯 TEST: US3 (BUG-020) - Library Display [REAL API]');
  ...
  test('US4 (BUG-019): Image metadata persists with originalParams for re-generation', async ({ page }) => {
    ...
    console.log('\n🎯 TEST: US4 (BUG-019) - Metadata Persistence [REAL API]');
  ...
  test('Metadata Validation: Invalid metadata is rejected or saved as null', async ({ page }) => {
    ...
    console.log('\n🎯 TEST: Metadata Validation [REAL API]');
  ...
  test('Console Logging: Navigation events and agent lifecycle are logged', async ({ page }) => {
    ...
    console.log('\n🎯 TEST: Console Logging Verification [REAL API]');
